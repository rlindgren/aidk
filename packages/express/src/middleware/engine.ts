 ...
 *       session_id: headers['x-session'],
 *       user_id: body.user.id,
 *       tenant_id: body.user.org_id,
 *     }),
 *   }),
 *   workflowRouter
 * );
 ...
 *   const { engine, input, execution_id } = req.engineContext;
...
export function withEngine<TBody = any>(
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    try {
      // Get engine instance
      const engine = typeof resolved.engine === 'function' 
        ? resolved.engine() 
        : resolved.engine;
      ...
      const execution_id = resolved.generateId();
      ...
      (req as EngineRequest).engineContext = {
        engine,
        execution_id,
        thread_id: requestContext.thread_id,
...
export interface TransportConfig {
  ...
  transport: SSETransport;
  /** 
   * Pattern for room name. Defaults to `thread:{thread_id}`.
   * @example (ctx) => `user:${ctx.user_id}:thread:${ctx.thread_id}`
   */
  roomPattern?: (ctx: RequestContext & { execution_id: string }) => string;
}
 ...
 *   transport,
 *   roomPattern: (ctx) => `org:${ctx.tenant_id}:thread:${ctx.thread_id}`,
 * }));
 * ```
 */
export function withTransport(
  ...
  const roomPattern = config.roomPattern || ((ctx) => `thread:${ctx.thread_id}`);
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (ctx.session_id && config.transport.isConnected(ctx.session_id)) {
        ...
        await config.transport.join(ctx.session_id, room);
      }
