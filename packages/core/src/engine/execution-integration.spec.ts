...
describe("Execution System Integration", () => {
  ...
  describe("multi-agent orchestration", () => {
    it("should orchestrate multiple agents with fork/spawn", async () => {
      ...
      // Spawn root agent
      const rootHandle = engine.spawn(createElement(AgentA, {}), { timeline: [] });
    ...
    it("should handle orphaned forks correctly", async () => {
      ...
      // Spawn main agent
      const mainHandle = engine.spawn(createElement(MainAgent, {}), { timeline: [] });
    ...
    it("should track outstanding forks", async () => {
      ...
      const parentHandle = engine.spawn(createElement(AgentA, {}), { timeline: [] });
  ...
  describe("execution graph integrity", () => {
    it("should maintain correct parent-child relationships", async () => {
      ...
      const root = engine.spawn(createElement(SimpleAgent, {}), { timeline: [] });
    ...
    it("should handle concurrent spawns correctly", async () => {
      ...
      const spawns = Array.from({ length: 5 }, () =>
        engine.spawn(createElement(SimpleAgent, {}), { timeline: [] }),
      );
      ...
      // All should complete
      const results = await Promise.all(spawns.map((s) => s.waitForCompletion({ timeout: 2000 })));
  ...
  describe("metrics accuracy", () => {
    ...
    it("should calculate average execution time correctly", async () => {
      ...
      await Promise.all(handles.map((h) => h.waitForCompletion({ timeout: 2000 })));
  ...
  describe("persistence integration", () => {
    ...
    it("should persist state with increasing tick numbers", async () => {
      ...
      await engine.execute.call({ timeline: [] }, createElement(SimpleAgent, {}));
  ...
  describe("long-running engine", () => {
    it("should handle many concurrent executions", async () => {
      ...
      // All should complete
      const results = await Promise.all(handles.map((h) => h.waitForCompletion({ timeout: 5000 })));
    ...
    it("should maintain graph integrity with many forks", async () => {
      ...
      // All should complete
      await Promise.all(forks.map((f) => f.waitForCompletion({ timeout: 5000 })));
