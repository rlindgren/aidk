...
describe('Engine v2', () => {
  ...
  describe('Basic execution', () => {
    it('should execute a simple agent', async () => {
      const mockComponent = defineComponent({
        render: () => createElement(Message, { role: 'user', content: [{ type: 'text', text: 'Hello' }] })
      });
    ...
    it('should support Procedure API (.use, .withHandle, .withContext)', async () => {
      ...
      const testMiddleware = async (args: [EngineInput, ComponentDefinition?], envelope: ProcedureEnvelope<[EngineInput, ComponentDefinition?]>, next: (transformedArgs?: [EngineInput, ComponentDefinition?]) => Promise<any>) => {
      ...
      // Test .withHandle()
      const { handle, result } = await engine.execute.withHandle().call({ timeline: [] }, mockComponent);
      ...
      if (resultWithContext && typeof resultWithContext === 'object' && 'then' in resultWithContext) {
  ...
  describe('Feature parity with Engine v1', () => {
    ...
    it('should handle tool calls', async () => {
      const toolCallModel = createModel<ModelInput, ModelOutput, ModelInput, ModelOutput, StreamChunk>({
        metadata: {
          id: 'tool-call-model',
          provider: 'mock',
          capabilities: []
        ...
        toEngineState: async (output: ModelOutput) => {
          return {
            newTimelineEntries: [],
            toolCalls: output.toolCalls?.map(tc => ({ id: tc.id, name: tc.name, input: tc.input })),
            usage: output.usage,
            shouldStop: false,
      ...
      const mockComponent = defineComponent({
        render: () => createElement(Message, { role: 'user', content: [{ type: 'text', text: 'Use tool' }] })
      });
    ...
    it('should execute a tool registered via components and verify results in timeline', async () => {
      ...
      const toolCallModel = createModel<ModelInput, ModelOutput, ModelInput, ModelOutput, StreamChunk>({
        ...
        toEngineState: async (output: ModelOutput) => {
          return {
            newTimelineEntries: output.message && output.message.content.length > 0 ? [{
            ...
            toolCalls: output.toolCalls?.map(tc => ({ id: tc.id, name: tc.name, input: tc.input })),
            usage: output.usage,
            shouldStop: !output.toolCalls?.length && ['stop', 'end_turn', 'max_tokens'].includes(output.stopReason as string),
      ...
      if (toolResultEntry) {
        const toolResultBlock = toolResultEntry.message.content.find((c: any) => c.type === 'tool_result');
  ...
  describe('Procedure-specific features', () => {
    it('should support middleware chaining', async () => {
      ...
      const mw1 = async (args: [EngineInput, ComponentDefinition?], envelope: ProcedureEnvelope<[EngineInput, ComponentDefinition?]>, next: (transformedArgs?: [EngineInput, ComponentDefinition?]) => Promise<any>) => {
      ...
      const mw2 = async (args: [EngineInput, ComponentDefinition?], envelope: ProcedureEnvelope<[EngineInput, ComponentDefinition?]>, next: (transformedArgs?: [EngineInput, ComponentDefinition?]) => Promise<any>) => {
    ...
    it('should propagate context through middleware', async () => {
      ...
      const mw = async (args: [EngineInput, ComponentDefinition?], envelope: ProcedureEnvelope<[EngineInput, ComponentDefinition?]>, next: (transformedArgs?: [EngineInput, ComponentDefinition?]) => Promise<any>) => {
