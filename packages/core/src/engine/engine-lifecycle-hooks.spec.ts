...
describe('Engine Lifecycle Hooks', () => {
  ...
  describe('onInit', () => {
    ...
    it('should call multiple onInit hooks in order', async () => {
      ...
      const engine = createEngine({
        ...
        lifecycleHooks: {
          onInit: [
            async () => { callOrder.push('hook1'); },
            async () => { callOrder.push('hook2'); },
            async () => { callOrder.push('hook3'); },
          ]
        }
      });
  ...
  describe('Hook Inheritance in Forks', () => {
    it('should inherit lifecycle hooks from parent engine', async () => {
      ...
      const parentEngine = createEngine({
        ...
        lifecycleHooks: {
          onExecutionStart: [
            async () => { parentHooksCalled.push('parent'); }
          ]
        }
      });
      ...
      expect(parentEngine.lifecycleHooks.getMiddleware('onExecutionStart').length).toBeGreaterThan(0);
    });
    ...
    it('should compose lifecycle hooks in forks', async () => {
      ...
      const forkHandle = parentEngine.fork(
        createElement(Fragment, null, 'Test'),
        input,
        {
  ...
  describe('Hook Registration Methods', () => {
    it('should support dynamic hook registration via methods', async () => {
      ...
      engine.onExecutionStart(async () => { hooksCalled.push('dynamic1'); });
      engine.onExecutionStart(async () => { hooksCalled.push('dynamic2'); });
    ...
    it('should return unregister function', async () => {
      ...
      const unregister = engine.onExecutionStart(async () => { hooksCalled.push('hook'); });
