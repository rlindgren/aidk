...
const toEngineStateMock = jest.fn(async (output: any) => ({
  ...
  shouldStop: !output.toolCalls?.length && ['stop', 'end_turn', 'max_tokens'].includes(output.stopReason),
...
describe('Engine React Architecture', () => {
  ...
  it('should render a simple functional component tree', async () => {
    const MyAgent = () => (
      <Section id="test" content="Hello World" />
    );
  ...
  it('should call lifecycle methods correctly during reconciliation', async () => {
    ...
    class LifecycleComp {
      ...
      render() { return <Section id="lifecycle" content="Exists" />; }
    }
        ...
        render() {
          return (
            <Fragment>
              ...
              {!this.showModel() && (
                <Message role="user" content="Tick 2 - no model" />
              )}
            </Fragment>
          );
        }
