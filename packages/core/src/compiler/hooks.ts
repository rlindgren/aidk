...
import { signal as createSignal, computed, createCOMStateSignal, createReadonlyCOMStateSignal, type Signal, type ComputedSignal, type ReadonlySignal, isSignal, isComputed } from '../state/signal';
...
export function useReducer<S, A>(
  ...
  if (hook.queue === null) {
    ...
    if (queue.pending !== null) {
      let update: Update<A> | null = queue.pending;
      const first = update;
      do {
...
export function useSignal<T>(initialValue: T): Signal<T> {
  ...
  if (hook.memoizedState === undefined) {
    hook.memoizedState = createSignal(initialValue);
  }
...
export function useAfterCompile(
  callback: (com: ContextObjectModel, compiled: CompiledStructure, state: TickState) => void
): void {
...
export function useAsync<T>(
  ...
  const depsChanged = prevDeps.current === null || 
    !areHookInputsEqual(deps, prevDeps.current);
