...
export function initializeContentBlockMappers(
  ...
  register(Text, (el, currentRenderer) => {
  ...
  // <p> - paragraph
  register('p' as any, (el, currentRenderer) => {
    const semanticNode = extractSemanticNodeFromElement(el, currentRenderer);
    return {
      type: 'text',
      text: '',
  ...
  register('blockquote' as any, (el, currentRenderer) => {
    const semanticNode = extractSemanticNodeFromElement(el, currentRenderer);
    return {
      type: 'text',
      text: '',
  ...
  register(Code, (el) => {
    const childrenText = el.props.children !== undefined
      ? (typeof el.props.children === 'string' ? el.props.children :
          Array.isArray(el.props.children) ? el.props.children.join('') : '')
      : undefined;
  ...
  register(H1, (el, currentRenderer) => {
    const semanticNode = extractSemanticNodeFromElement(el, currentRenderer);
    return {
      type: 'text',
      text: '', // Will be populated by renderer from semanticNode
  ...
  register(H2, (el, currentRenderer) => {
    const semanticNode = extractSemanticNodeFromElement(el, currentRenderer);
    return {
      type: 'text',
      text: '', // Will be populated by renderer from semanticNode
  ...
  register(H3, (el, currentRenderer) => {
    const semanticNode = extractSemanticNodeFromElement(el, currentRenderer);
    return {
      type: 'text',
      text: '', // Will be populated by renderer from semanticNode
  ...
  register(Header, (el, currentRenderer) => {
  ...
  // Paragraph
  register(Paragraph, (el, currentRenderer) => {
    const semanticNode = extractSemanticNodeFromElement(el, currentRenderer);
    return {
      type: 'text',
      text: '', // Will be populated by renderer from semanticNode
