...
import type { TickState, AfterCompileContext, RecoveryAction, EngineComponent } from '../component/component';
...
import { createFiber, createWorkInProgress, getChildFibers, traverseFiber, traverseFiberBottomUp } from './fiber';
...
export class FiberCompiler {
  ...
  constructor(com: ContextObjectModel, hookRegistry?: ComponentHookRegistry, config: FiberCompilerConfig = {}) {
    this.com = com;
  ...
  async compileUntilStable(
    ...
    do {
      ...
      const ctx: AfterCompileContext = { iteration: iterations, maxIterations };
      ...
      if (iterations >= maxIterations) {
        log.warn({ maxIterations, reasons: recompileReasons }, 
  ...
  private async updateFunctionComponent(
    ...
    try {
      ...
      if (Component.length >= 3) {
        children = await (Component as (p: unknown, c: ContextObjectModel, s: TickState) => FiberChild | Promise<FiberChild>)(
          props, this.com, this.tickState!
        );
      } else if (Component.length === 2) {
        children = await (Component as (p: unknown, c: ContextObjectModel) => FiberChild | Promise<FiberChild>)(
          props, this.com
        );
      } else {
      ...
      const isSelfReferential = isElement(children) && (
      ...
      if (children !== null && children !== undefined) {
        if (isElement(children)) {
          ...
          const childFiber = await this.reconcileElement(fiber, oldChildFiber, children as JSX.Element, 0);
          if (childFiber) {
  ...
  private isClassComponent(type: unknown): type is ClassComponent {
    ...
    while (current && current !== Object.prototype) {
      if (current.constructor?.name === 'Component' ||
  ...
  /**
   * Check if a type is a primitive/host component (Section, Entry, etc.)
  ...
  private isHostPrimitive(type: unknown): boolean {
    ...
    // Check by reference or name for cross-module compatibility
    ...
    return hostPrimitives.some(p => 
      type === p.ref || 
      (type as { name?: string }).name === p.name
    );
  }
  ...
  private registerContentBlock(type: unknown, mapper: ContentBlockMapper, stringType?: string): void {
    this.contentBlockMappers.set(type, mapper);
    const typeName = stringType || 
      (typeof type === 'function' ? (type as { name?: string }).name?.toLowerCase() : String(type).toLowerCase());
    if (typeName) {
  ...
  private traverseAndCollect(
    ...
    const currentRenderer = rendererStack.length > 0
    ...
    if (this.isType(type, Entry, 'Entry')) {
      ...
      if (props.kind === 'message') {
        const messageProps = props.message as Record<string, unknown> | undefined;
        const role = messageProps?.role as CompiledTimelineEntry['message'] extends { role: infer R } ? R : string;
        ...
        if (role === 'system') {
          ...
          const entry: CompiledTimelineEntry = {
            kind: 'message',
            message: {
              role: (role || 'user') as CompiledTimelineEntry['message'] extends { role: infer R } ? R : never,
              content,
            },
            tags: props.tags as string[] | undefined,
    ...
    if (this.isType(type, Tool, 'Tool')) {
      if (props.definition) {
        const toolDef = typeof props.definition === 'string'
        ...
        if (toolDef) {
          ...
          if (toolAny.metadata) {
            ...
            if (name) {
              ...
              const toolEntry = { name, tool: toolDef as CompiledStructure['tools'][number]['tool'] };
              if (existingIndex >= 0) {
  ...
  private isContentType(type: unknown): boolean {
    return type === Text || type === Code || type === Image ||
           type === Json || type === Document || type === Audio || type === Video ||
           type === 'text' || type === 'code' || type === 'image' ||
           type === 'json' || type === 'document' || type === 'audio' || type === 'video';
  }

  private collectContentFromFiber(fiber: FiberNode, renderer?: ContentRenderer): SemanticContentBlock[] {
  ...
  private collectContentFromChild(
    ...
    if (typeof type === 'string') {
      const semanticNode = extractSemanticNodeFromElement({ type, props } as JSX.Element);
      blocks.push({
        type: 'text',
        text: '',
        semanticNode,
