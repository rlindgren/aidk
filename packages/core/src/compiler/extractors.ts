 ...
 * ```
 */
export function extractSemanticNode(fiber: FiberLike, currentRenderer?: ContentRenderer): SemanticNode {
  ...
  const hasRendererInChildren = children.some(n => 
 ...
 */
export function extractSemanticNodeFromElement(element: JSX.Element, currentRenderer?: ContentRenderer): SemanticNode {
  const extract = (node: any, rendererContext?: ContentRenderer): SemanticNode | SemanticNode[] => {
    if (typeof node === 'string') {
      return { text: node };
    }
    ...
    if (node && typeof node === 'object') {
      if (isElement(node)) {
        ...
        if (semanticType) {
          ...
          } else if ((semanticType === 'image' || semanticType === 'audio' || semanticType === 'video') && node.props) {
  ...
  if (props.children !== undefined && props.children !== null) {
    ...
    if (extracted.length === 0) {
      return currentRenderer && currentRenderer !== undefined ? { renderer: currentRenderer, children: [] } : { text: '' };
    }
    
    // If we have renderer context and root doesn't have renderer, attach it
    const rootNode: SemanticNode = { children: extracted };
    if (currentRenderer && !extracted.some((n: any) => n && typeof n === 'object' && 'renderer' in n)) {
      rootNode.renderer = currentRenderer;
    }
 ...
 */
function extractRowData(rowElement: JSX.Element): { cells: string[]; alignments: ('left' | 'center' | 'right')[] } {
  const cells: string[] = [];
  const alignments: ('left' | 'center' | 'right')[] = [];
  
  const props = rowElement.props || {};
  const rawChildren = props.children;
  const children = Array.isArray(rawChildren) ? rawChildren.flat() : (rawChildren ? [rawChildren] : []);
 ...
 */
export function extractTableStructure(tableElement: JSX.Element): { headers: string[]; rows: string[][]; alignments?: ('left' | 'center' | 'right')[] } {
  ...
  return { headers, rows, alignments: alignments.length > 0 ? alignments : undefined };
}

/**
 * Extracts data from a ListItem element, including nested lists.
 */
function extractListItemData(itemElement: JSX.Element): string | { text: string; nested?: any } {
 ...
 */
export function extractListStructure(listElement: JSX.Element): { ordered: boolean; items: (string | { text: string; nested?: any })[] } {
  const props = listElement.props || {};
  const ordered = props.ordered === true;
  const items: (string | { text: string; nested?: any })[] = [];
