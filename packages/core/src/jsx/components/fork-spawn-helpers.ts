...
export interface CreateForkOptions {
  /**
   * Root definition - either a ComponentDefinition or JSX children
   */
  root?: ComponentDefinition | JSX.Element | JSX.Element[];
...
export function createForkHandle(
  ...
  // Normalize root definition
  const rootDefinition = normalizeRootDefinition(options.root);
  if (!rootDefinition) {
    throw new Error("Cannot fork: root or children must be provided");
  }
  ...
  if (!parentPid) {
    throw new Error(
      "Cannot fork: no parent execution found. Provide parentPid or call from within an execution context.",
    );
  }

  // Create fork handle
  const handle = engine.fork(rootDefinition, options.input || { timeline: [] }, {
    parentPid,
 ...
 *       state,
 *       {
 *         root: createElement(IndependentRoot, {}),
 *         input: { timeline: [] },
 *         onComplete: (result) => {
...
export function createSpawnHandle(
  ...
  const rootDefinition = normalizeRootDefinition(options.root);
  if (!rootDefinition) {
    throw new Error("Cannot spawn: root or children must be provided");
  }

  // Create spawn handle
  const handle = engine.spawn(rootDefinition, options.input || { timeline: [] }, {
    engineConfig: options.engineConfig,
  });
...
export function registerWaitHandle(
  ...
  handle.waitForCompletion().finally(() => {
 ...
 *
 * @param root Root definition (ComponentDefinition or JSX children)
 * @returns Normalized ComponentDefinition
 */
function normalizeRootDefinition(
  root?: ComponentDefinition | JSX.Element | JSX.Element[],
  ...
  if (typeof root === "function" || (typeof root === "object" && "render" in root)) {
    return root as ComponentDefinition;
  }

  // If it's JSX.Element(s), wrap in Fragment
  if (Array.isArray(root)) {
    return createElement(Fragment, {}, ...root);
  }
