...
describe('COMInput Validation', () => {
  ...
  describe('Basic Structures', () => {
    ...
    it('should collect messages from Message components', async () => {
      ...
      expect(result.timeline[0].message.content[0]).toEqual({ type: 'text', text: 'First message' });
      expect(result.timeline[1].message.role).toBe('user');
      expect(result.timeline[1].message.content[0]).toEqual({ type: 'text', text: 'Second message' });
    });
  ...
  describe('System Messages', () => {
    it('should collect system messages and send to model', async () => {
      ...
      const systemMessage = modelInput.messages?.find((m: any) => m.role === 'system');
      expect(systemMessage).toBeDefined();
      expect(systemMessage?.content[0]).toEqual({ type: 'text', text: 'You are a helpful assistant' });
    });
  ...
  describe('Multiple Components Contributing', () => {
    ...
    it('should merge messages from multiple components', async () => {
      ...
      expect(userMessages[0].message.content[0]).toEqual({ type: 'text', text: 'From Component A' });
      expect(userMessages[1].message.content[0]).toEqual({ type: 'text', text: 'From Component B' });
    });
  ...
  describe('Conditional Rendering', () => {
    ...
    it('should handle conditionally rendered messages', async () => {
      ...
      expect(userMessages[1].message.content[0]).toEqual({ type: 'text', text: 'Conditionally shown' });
    });
  });
  ...
  describe('Multiple Models Conditionally Rendered', () => {
    ...
    const accurateModel = createModel<ModelInput, ModelOutput, ModelInput, ModelOutput, StreamChunk>({
  ...
  describe('Visibility and Tags', () => {
    ...
    it('should preserve tags on timeline entries', async () => {
      ...
      const taggedMsg = result.timeline.find(t => 
        t.message.role === 'user' && 
