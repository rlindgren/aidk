...
describe("Compilation Integration Tests", () => {
  ...
  describe("Text Wrapping", () => {
    ...
    it("should wrap unwrapped strings in messages as TextBlocks", async () => {
      const jsx = <Message role="user">Plain string message</Message>;
    ...
    it("should handle mixed wrapped and unwrapped content", async () => {
      const jsx = (
        <Message role="user">
          Start text
          <Text>
            Middle <strong>formatted</strong> text
          </Text>
          End text
        </Message>
      );
  ...
  describe("Semantic Component Rendering", () => {
      ...
      const jsx = (
        <Message role="user">
          <Text>
            ...
            <code>code</code> <mark>marked</mark> <u>underlined</u> <s>strikethrough</s>{" "}
            <sub>subscript</sub> <sup>superscript</sup> <small>small</small>
          </Text>
        </Message>
      );
    ...
    it("should render headings with formatting", async () => {
      const jsx = (
        <>
          <Message role="user">
            <H1>
              Title with <strong>bold</strong>
            </H1>
            <H2>
              Subtitle with <em>italic</em>
            </H2>
            <H3>
              Section with <code>code</code>
            </H3>
          </Message>
        </>
      );
    ...
    it("should render semantic HTML elements", async () => {
      const jsx = (
        <Message role="user">
          <Text>
            Link: <a href="https://example.com">Click here</a> Quote: <q>quoted text</q> Citation:{" "}
            <cite>Reference</cite> Keyboard: <kbd>Ctrl+C</kbd> Variable: <var>x</var>
          </Text>
        </Message>
      );
    ...
    it("should render block elements with nested formatting", async () => {
      ...
      expect((result.timeline[0].message.content[1] as any).text).toContain("Quote with `code`");
    });
    ...
    it("should render inline media elements (img, audio, video)", async () => {
      const jsx = (
        <Message role="user">
          <Text>
            See this: <img src="photo.jpg" alt="Photo" /> Listen: <audio src="sound.mp3" /> Watch:{" "}
            <video src="video.mp4" />
          </Text>
        </Message>
      );
  ...
  describe("Structural vs Semantic Media", () => {
    ...
    it("should convert semantic <img> to inline markdown", async () => {
      const jsx = (
        <Message role="user">
          <Text>
            Inline image: <img src="photo.jpg" alt="Photo" />
          </Text>
        </Message>
      );
  ...
  describe("Renderer Component Behavior", () => {
    it("should use default MarkdownRenderer by default", async () => {
      ...
      expect((result.timeline[0].message.content[0] as any).text).toBe("Hello **world**");
    });
    ...
    it("should handle mixed renderers (XML inside Markdown)", async () => {
      const jsx = (
        <Markdown>
          <Message role="user">
            <Text>
              Markdown: <strong>bold</strong>
            </Text>
          </Message>
          <XML>
            <Message role="assistant">
              <Text>
                XML: <strong>bold</strong>
              </Text>
            </Message>
          </XML>
        </Markdown>
      ...
      expect((result.timeline[0].message.content[0] as any).text).toBe("Markdown: **bold**");

      // Second message uses XML (overrides parent Markdown)
      expect((result.timeline[1].message.content[0] as any).text).toBe(
        "XML: <strong>bold</strong>",
      );
    });
    ...
    it("should support nested renderer switches", async () => {
      const jsx = (
        <XML>
          <Section id="xml-section">
            <Text>
              XML content: <em>italic</em>
            </Text>
          </Section>
          <Markdown>
            <Section id="md-section">
              <Text>
                Markdown content: <em>italic</em>
              </Text>
            </Section>
          </Markdown>
        </XML>
      ...
      // XML section
      expect((result.sections["xml-section"].content as any[])[0].text).toBe(
        "XML content: <em>italic</em>",
      );

      // Markdown section (nested override)
      expect((result.sections["md-section"].content as any[])[0].text).toBe(
  ...
  describe("Edge Cases", () => {
    ...
    it("should preserve metadata and tags", async () => {
      const jsx = (
        <Message role="user" tags={["test", "important"]} metadata={{ custom: "value" }}>
          <Text>Content</Text>
        </Message>
      );
  ...
  describe("Code Block Handling", () => {
    ...
    it("should handle inline <code> differently", async () => {
      const jsx = (
        <Message role="user">
          <Text>
            Use <code>const x = 42;</code> for assignment
          </Text>
        </Message>
      );
    ...
    it("should pass through native code blocks unchanged", async () => {
      ...
      const jsx = <Message role="user">{nativeCodeBlock}</Message>;
  ...
  describe("Special Content Block Types", () => {
    it("should pass through reasoning blocks", async () => {
      ...
      const jsx = <Message role="assistant">{reasoningBlock}</Message>;
    ...
    it("should pass through tool_use blocks", async () => {
      ...
      const jsx = <Message role="assistant">{toolUseBlock}</Message>;
    ...
    it("should pass through tool_result blocks", async () => {
      ...
      const jsx = <Message role="user">{toolResultBlock}</Message>;
  ...
  describe("Complex Real-World Scenarios", () => {
    it("should handle conversation with system + user + assistant", async () => {
      ...
      // System message
      expect(result.system[0].message.role).toBe("system");
      expect((result.system[0].message.content[0] as any).text).toContain(
    ...
    it("should handle paragraph with inline media", async () => {
      const jsx = (
        <Message role="user">
          <p>
            See the diagram <img src="diagram.png" alt="Diagram" /> for details. Also check{" "}
            <a href="https://docs.example.com">the documentation</a>.
          </p>
        </Message>
      );
  ...
  describe("Event messages types", () => {
    it("should handle event messages", async () => {
      ...
      const rawContent = result.compiled.timelineEntries?.[0]?.message
        ?.content[0] as UserActionBlock;
