          ...
          timeline: [
            {
              ...
              message: {
                role: 'event',
                content: [{ type: 'user_action', action: 'view_invoice', actor: 'user', details: { invoice_id: '123' } }],
              } as EventMessage,
            },
            {
              kind: 'message',
  ...
  describe('toEngineState', () => {
    ...
    describe('message array handling', () => {
      ...
      it('should filter out tool role messages from timeline entries', async () => {
        ...
        expect(result.newTimelineEntries).toHaveLength(2);
        expect(result.newTimelineEntries!.every(e => (e.message as Message).role !== 'tool')).toBe(true);
      });
    });
    ...
    describe('backward compatibility', () => {
      it('should merge legacy toolCalls with extracted tool calls', async () => {
        const output: ModelOutput = {
          ...
          toolCalls: [
            { id: 'legacy_1', name: 'legacy_tool', input: { x: 1 } },
          ],
        };
