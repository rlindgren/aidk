...
import type { ContentBlock, Message, EventBlock, TextBlock, AgentToolResult, AgentToolCall } from "aidk-shared";
...
function resolveTransformationConfig(
  ...
  modelCapabilities?: { messageTransformation?: MessageTransformationConfig | ((modelId: string, provider?: string) => MessageTransformationConfig) },
  ...
  if (modelCapabilities?.messageTransformation) {
    ...
    baseConfig = typeof cap === 'function' 
      ? cap(modelId, provider)
      : cap;
  }
 ...
 */
function isBlockDelimiters(delimiter: DelimiterConfig | EventBlockDelimiters | undefined): delimiter is EventBlockDelimiters {
  if (!delimiter || typeof delimiter === 'string') return false;
  // Check if it has any of the block-type keys
  return 'user_action' in delimiter || 'system_event' in delimiter || 
         'state_change' in delimiter || 'default' in delimiter;
}
 ...
 */
function wrapTextWithDelimiter(text: string, delimiter: DelimiterConfig | undefined): ContentBlock[] {
  if (!delimiter) {
  ...
  if (startDelim) {
    result.push({ type: 'text' as const, text: `${startDelim} ${text}${endDelim ? ` ${endDelim}` : ''}` });
  } else {
...
function convertUnsupportedBlocksToText(blocks: ContentBlock[]): ContentBlock[] {
  return blocks.flatMap(block => {
    ...
    const unsupportedTypes = ['xml', 'csv', 'html', 'generated_image', 'generated_file', 
...
function transformEventContent(
  ...
  const allText = content.map(getEventBlockText).filter(text => text && text.trim() !== '').join('\n');
  ...
  if (config.delimiters?.useDelimiters && config.delimiters.ephemeral) {
    ...
    if (startDelim) {
      if (content.length > 0 && content[0].type === 'text') {
        content[0] = { type: 'text' as const, text: `${startDelim}\n${(content[0] as TextBlock).text}` };
      } else {
    ...
    if (endDelim) {
      ...
      if (lastIdx >= 0 && content[lastIdx].type === 'text') {
        content[lastIdx] = { type: 'text' as const, text: `${(content[lastIdx] as TextBlock).text}\n${endDelim}` };
      } else {
...
export async function fromEngineState(
  input: COMInput, 
  modelOptions?: ModelConfig,
  model?: { metadata: { id?: string; model?: string; provider?: string; capabilities?: { messageTransformation?: MessageTransformationConfig | ((modelId: string, provider?: string) => MessageTransformationConfig) } } }
): Promise<ModelInput> {
  ...
  const conversationMessages = timelineMessages.map(msg => {
    ...
    if (msg.role === 'event') {
      ...
      // Map 'developer' to 'user' for Message type (adapter will convert to 'developer' if supported)
      const messageRole: Message['role'] = eventRole === 'developer' ? 'user' : 
                                           eventRole === 'event' ? 'user' : 
  ...
  const messages = interleaveEphemeral(
    baseMessages, 
    input.ephemeral || [],
...
export async function toEngineState(output: ModelOutput): Promise<EngineResponse> {
  ...
  return {
    ...
    usage: output.usage,
    shouldStop: allPendingToolCalls.length === 0 && stopReasonInfo
