...
export type SemanticType = 
  // Inline formatting
  | 'strong' | 'em' | 'mark' | 'underline' | 'strikethrough' | 'subscript' | 'superscript' | 'small' | 'code'
  // Block formatting
  | 'heading' | 'list' | 'table' | 'paragraph' | 'blockquote' | 'line-break' | 'horizontal-rule'
...
export abstract class Renderer {
    ...
    for (const block of blocks) {
      ...
      if (block.semanticNode) {
        ...
        if (node.renderer) {
          ...
          const finalText = (node.renderer as any).applyBlockLevelFormatting?.(block, formattedText) || formattedText;
      ...
      const isEventBlock = block.type === 'user_action' || block.type === 'system_event' || block.type === 'state_change';
  ...
  protected abstract applyBlockLevelFormatting(block: SemanticContentBlock, formattedText: string): string;
}
