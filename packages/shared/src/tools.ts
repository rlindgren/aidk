...
export interface AgentToolResult {
   ...
   */
  executed_by?: 'engine' | 'provider' | 'adapter' | 'client';
...
export interface ToolDefinition {
   ...
   * Only applicable when requires_response is true.
   * Default: 30000 (30 seconds)
   */
  timeout?: number;
  /**
   * Default result to use when requires_response is false.
   * Returned immediately for render tools that don't need client feedback.
   * Default: [{ type: 'text', text: '[{name} rendered on client]' }]
   */
  default_result?: ContentBlock[];
}
...
export interface ClientToolDefinition {
  ...
  timeout?: number;
  /**
   * Default result when requires_response is false.
   * Default: [{ type: 'text', text: '[{name} rendered on client]' }]
   */
  default_result?: ContentBlock[];
}
