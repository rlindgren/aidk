...
> {
   ...
   */
  send<K extends keyof TOutgoing>(eventType: K, payload: TOutgoing[K]): Promise<unknown>;
...
> implements Channel<TIncoming, TOutgoing> {
  ...
  private ensureConnected(): void {
    ...
    this.unsubscribe = this.client.subscribe(this.name, (event) => {
      ...
      if (this.name === "todo-list") {
        console.debug(
          `[Channel:${this.name}] Received event:`,
          event.type,
          "handlers:",
          this.handlers.get(event.type)?.size || 0,
        );
      }
  ...
  async send<K extends keyof TOutgoing>(eventType: K, payload: TOutgoing[K]): Promise<unknown> {
