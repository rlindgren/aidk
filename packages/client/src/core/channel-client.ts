...
export class ChannelClient {
  ...
  private publishOverride?: <T = unknown>(channel: string, type: string, payload?: unknown) => Promise<T>;
  ...
  subscribe(
    ...
    for (const channel of channels) {
      ...
      if (channel === 'todo-list') {
        console.log(`[ChannelClient] Subscribed to ${channel}, total handlers:`, this.handlers.get(channel)?.size);
      }
    }
  ...
  private handleMessage(data: unknown): void {
    ...
    if (event.channel === 'todo-list') {
      console.log(`[ChannelClient] Received event for ${event.channel}:`, event.type, 'handlers:', this.handlers.get(event.channel)?.size || 0);
    }
