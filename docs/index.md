...
Forks and spawns can receive messages mid-execution:
...
// Send a message into the running execution
await handle.send({
  type: 'user_input',
  content: 'Actually, change the approach',
});

// Agent receives it via channel or event
class InteractiveAgent extends Component {
  onMessage(com: ContextObjectModel, message: ExecutionMessage) {
    // React to incoming message
    com.setState('userOverride', message.content);
  }
}
```
