import { Section, Logger, Paragraph, comState, computed, Model, COM, TickState } from "aidk";
...
export class TaskAssistantAgent {
  ...
  private openaiModel = comState<string>(
    "openaiModel",
    process.env["OPENAI_MODEL"] || "gpt-4o-mini",
  );
  private googleModel = comState<string>(
    "googleModel",
  ...
  private executionModel = computed(() =>
    aisdk({
      model: this.model(),
  ...
  async render(com: COM, state: TickState) {
    return (
      <>
        ...
        <Section id="instructions" title="Instructions" audience="model">
          <Paragraph>
            You are a <strong>helpful</strong> assistant with access to tools.
          </Paragraph>
          <Paragraph>
            When asked to perform actions, use the appropriate tool immediately. Do NOT describe
            what you would or will do - you MUST call the tool.
          </Paragraph>
  ...
  private getModelProviderOptions(
    ...
    return {
      google: {
        thinkingConfig: {
          ...
          thinkingBudget:
            reasoning.effort === "low" ? 1024 : reasoning.effort === "medium" ? 2048 : 4096,
        },
      },
      openai: {
