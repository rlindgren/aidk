...
export const ScratchpadTool = createTool({
  name: 'scratchpad',
  description: 'A scratchpad for quick notes during this conversation. Use it to jot down important points, track context, or organize your thoughts. Notes are visible to both you and the user.',
  ...
  handler: async (input: ScratchpadInput): Promise<ContentBlock[]> => {
    ...
    switch (input.action) {
      ...
      case 'list':
        ...
        break;
      default:
        result = { success: false, message: `Unknown action: ${input.action}. Please use one of the following actions: add, remove, clear, list.` };
        break;
    }
  ...
  render(com: COM, state: TickState): JSX.Element | null {
    ...
    return (
      <>
        ...
        >
          <Paragraph>You have a <inlineCode>scratchpad</inlineCode> tool for taking notes during this conversation. Use it to jot down important points, track context, or organize your thoughts. Notes are visible to both you and the user.</Paragraph>
        ...
        {notes.length === 0 ? (
          ...
          >
            <Paragraph><strong>Current Scratchpad State:</strong> You have NO notes saved. The scratchpad is empty.</Paragraph>
          </Grounding>
        ) : (
          <Grounding
            position="after-system"
            ...
            tags={['scratchpad', 'state']}
          >
            <Paragraph><strong>Current Scratchpad State:</strong> You have {notes.length} active note(s) saved:</Paragraph>
            <List ordered>
            ...
            <Paragraph><em>These notes are currently active and visible to the user. When the user asks about notes, these are the notes they are referring to.</em></Paragraph>
          </Grounding>
        )}
      </>
    );
